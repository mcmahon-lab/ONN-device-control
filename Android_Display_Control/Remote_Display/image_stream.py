# Send and Display Byte Arrays to Image Stream Andriod App
#
#  Generates an array corresponding to an image with the top half RED
#  and the bottom half WHITE. Intended to serve as an example of how to
#  send arbitrary byte arrays to the Image Stream Android app. Images must
#  be 1920x1080--pad with zeros if necessary.
#
...
class AndroidOpenAccessoryBridge:
    ...
    def write(self, data, timeout=None):
        ...
        while True:
            try:
                bytes_wrote = self._endpoint_out.write(size_bytes, timeout=timeout)
...
if __name__ == '__main__':
    ...
    # create a test image to send
    numBytes = 1920*1080*3
    pixelVals = bytearray()
    for i in range(numBytes):
      if(i//(1920*3*540) == 0):
        if(i%3==0):
          pixelVals.append(0xff)
        else:
          pixelVals.append(0x00)
      else:
        pixelVals.append(0xff)
    ...
    with AndroidOpenAccessoryBridge(
        ...
        aoab.write(pixelVals) #write the pixel values to the Android app to be displayed
